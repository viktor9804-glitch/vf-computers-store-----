  ...
  const sendOrder = async (checkoutForm) => {
    ...
    const orderPayload = {
      ...
      items: cartItems.map((item) => ({
        ...
        image: item.image || item.image_url || item.thumbnail || item.main_image || item.images?.[0] || "",
        image_url: item.image_url || item.image || item.thumbnail || item.main_image || item.images?.[0] || "",
        images: item.images || (item.image ? [item.image] : []),
        catalog_number: item.catalog_number || item.catalogNumber || item.catalogNo || item.catalog || null,
        sku: item.sku || null,
        ...
        warranty: item.warranty || item.warranty_months || null,
        warranty_months: item.warranty_months || null,
        description: item.description || null,
        specs: item.specs || null,
        characteristics: item.characteristics || item.filters || null,
